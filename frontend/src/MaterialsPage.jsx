...
export default function MaterialsPage() {
  ...
  return <section className="materials-widget">
    ...
    <p>품목별 통화·중량 단위와 가격 규격을 확인하세요. 아래 기간은 확보된 이력 범위 내에서 표시됩니다.</p>{item && <div className="material-detail"><div className="material-chart-head"><div><strong>{item.name} · 공개 가격 추이</strong><p>{item.grade} / {item.unit}</p></div><div>{[30,90,180,365].map(d=><button key={d} className={days===d?'selected':''} onClick={()=>setDays(d)}>{d===365?'1년':`${d}일`}</button>)}</div></div>
