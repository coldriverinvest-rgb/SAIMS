...
def resolve_stock_code(corp_name: str) -> str:
    ...
    try:
        ...
        logger.warning("종목코드 조회 실패: %s", corp_name, exc_info=True)
        return ""
...
def fetch_stock_analysis(corp_name: str, period: str = "1y") -> dict:
    ...
    try:
        ...
        history_count = max(320, period_count + 119)
        response = requests.get(
            "https://fchart.stock.naver.com/sise.nhn",
            params={"symbol": stock_code, "timeframe": "day", "count": history_count, "requestType": 0},
        ...
        logger.warning("주가 데이터 조회/파싱 실패: %s (%s)", corp_name, stock_code, exc_info=True)
        return {}
